...
function SidebarItem({
  ...
  return (
    ...
    >
      ...
      {!collapsed && (
        <Text style={[sb.label, isActive && sb.labelActive]}>
          {label}
        </Text>
      )}
    ...
    <View style={tb.item}>
      <View style={[tb.iconWrap, focused && tb.iconWrapActive]}>
        {Icon ? (
          <Icon
            size={19}
            color={focused ? "#fff" : Colors.text3}
          />
        ) : (
    ...
    <View style={pa.root}>
      <View style={pa.card}>
        <View
          style={[
            pa.iconCircle,
            suspended && pa.iconCircleSuspended,
          ]}
        >
          <Text style={pa.icon}>
            {suspended ? "⛔" : "⏳"}
...
export default function TabsLayout() {
  ...
  const setDataViewMode = useStore(
    (s) => s.setDataViewMode
  );
  ...
  const hasViewToggle =
    hasAdminViewToggle(currentUserRole);
  ...
  // Sidebar collapse — web/desktop only
  ...
  const offlineBanner = !isOnline ? (
    <View style={shared.offlineBanner}>
      <Text style={shared.offlineBannerText}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Wallet and Investments are accessible by route
            but excluded from the mobile tab bar. */}
        <Tabs.Screen
          name="wallet"
          options={{ href: null }}
        />
...
const tb = StyleSheet.create({
  bar: {
    ...
    borderTopColor: Colors.border,
    height:
      Platform.OS === "ios" ? 82 : 66,
    paddingHorizontal: 2,
    paddingTop: 4,
    paddingBottom:
      Platform.OS === "ios" ? 20 : 4,
    shadowColor: "#000",
    shadowOffset: {
      width: 0,
      height: -2,
